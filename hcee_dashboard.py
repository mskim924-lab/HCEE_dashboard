...
def generate_signal():
    ...
    # ===== Trigger auto-recovery cycle if any trigger is now engaged =====
    # The dashboard will advance ticks automatically (one per ~0.7 sec)
    # until all triggers release, visualizing the closed-loop recovery.
    if (s.tc01_phase == "engaged" or s.tc02_phase == "engaged" or s.tc03_active):
        s.recovery_in_progress = True
        s.recovery_tick_count = 0
...
def force_exi_shock(magnitude=15):
    ...
    # ===== After EXI shock, ALWAYS trigger auto-recovery cycle =====
    # The shock creates state requiring closed-loop recovery (TC-02 + TC-03 engaged).
    # Dashboard auto-advances ticks until all triggers release.
    s.recovery_in_progress = True
    s.recovery_tick_count = 0
