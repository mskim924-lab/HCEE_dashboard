...
TC03_ACTIVE_TIMEOUT = 5   # ticks - shorter window for the cross-domain divergence trigger
...
PXI_SUBS = [
    {"id": "pxi_comm",     "name": "AI-Assisted Communication Clarity",   "name_kr": "AI 보조 설명 명확성",
    ...
    {"id": "pxi_ai_trust", "name": "AI Decision Trust",       "name_kr": "AI 의사결정 신뢰",
     ...
     "linked_ai": ["AI-001", "AI-002", "AI-003"]},
    {"id": "pxi_response", "name": "AI-Enabled Responsiveness",          "name_kr": "AI 기반 응대 적시성",
     "source": "Daily EMR Metrics",                   "source_kr": "일일 EMR 지표",
     "cadence": "daily",           "dept": "원무부",             "color": "#2D7A60",
     "linked_ai": ["AI-006"]},
    {"id": "pxi_symptom",  "name": "AI-Supported Symptom Detection",      "name_kr": "AI 보조 증상 감지",
     "source": "Daily Clinical Data",                 "source_kr": "일일 임상 데이터",
     "cadence": "daily",           "dept": "약제부·통증관리팀",  "color": "#6B8E23",
     "linked_ai": ["AI-002", "AI-005"]},
    {"id": "pxi_info",     "name": "AI-Generated Information Quality",     "name_kr": "AI 생성 정보 품질",
     "source": "Weekly + Discharge Feedback",         "source_kr": "주간 + 퇴원 피드백",
     "cadence": "weekly+pulse",    "dept": "의료진",             "color": "#4682B4",
     "linked_ai": ["AI-005", "AI-004"]},
    {"id": "pxi_empathy",  "name": "Human-AI Care Balance",       "name_kr": "인간-AI 돌봄 균형",
     "source": "Weekly Survey · HIRA",                "source_kr": "주간 설문 · HIRA",
     "cadence": "weekly",          "dept": "의료진 전체",        "color": "#3D6B47",
     "linked_ai": ["AI-001", "AI-003"]},
]

# ===== EXI Sub-dimensions (6 dims, Maslach + AI-era extensions) =====
EXI_SUBS = [
    {"id": "exi_workload",  "name": "AI-Related Cognitive Load",      "name_kr": "AI 관련 인지 부담",
     "source": "Daily EMR Usage Logs",                "source_kr": "일일 EMR 사용 로그",
     "cadence": "daily",           "dept": "각 진료과",          "color": "#7A2E2E",
     "linked_ai": ["AI-004", "AI-005"]},
    {"id": "exi_usability", "name": "AI Tool Usability",       "name_kr": "AI 도구 사용성",
     ...
     "linked_ai": ["AI-001", "AI-002", "AI-003", "AI-004", "AI-005", "AI-006"]},
    {"id": "exi_autonomy",  "name": "Autonomy under AI Oversight",       "name_kr": "AI 감독 하 임상 자율성",
     "source": "Weekly Pulse Survey",                 "source_kr": "주간 펄스 설문",
     "cadence": "weekly",          "dept": "의무위·인사부",      "color": "#B4953C",
     "linked_ai": ["AI-001", "AI-003"]},
    {"id": "exi_safety",    "name": "AI Challenge Safety",    "name_kr": "AI 이의제기 안전성",
...
/* ===== Overview tab section headers & cell cards ===== */
.overview-sec-header {{
  margin-top: 22px;
  margin-bottom: 14px;
  padding: 10px 16px;
  background: linear-gradient(135deg, #F4F0E4 0%, #FAF6E8 100%);
  border-left: 6px solid {C_TITLE};
  border-radius: 4px;
}}
.overview-sec-num {{
  display: inline-block;
...
.overview-sec-title-en {{
  font-weight: 700;
  color: {C_TITLE};
  font-size: 1.06em;
  vertical-align: middle;
}}
.overview-sec-title-kr {{
  font-size: 0.82em;
  color: {C_SUB};
  margin-left: 8px;
  vertical-align: middle;
}}
.overview-sec-caption {{
  font-size: 0.76em;
  color: {C_SUB};
  margin-top: 6px;
  line-height: 1.4;
...
.overview-cell-header-daily   {{ border-top-color: {C_PXI}; }}
.overview-cell-header-ai      {{ border-top-color: {C_EXI}; }}
.overview-cell-header-drivers {{ border-top-color: {C_STAB}; }}
.overview-cell-header-learning{{ border-top-color: #4A6FA5; }}
.overview-cell-icon {{
  font-size: 1.1em;
  margin-right: 6px;
}}
.overview-cell-title-en {{
  font-weight: 700;
  color: {C_TITLE};
  font-size: 0.96em;
}}
.overview-cell-title-kr {{
  font-size: 0.75em;
  color: {C_SUB};
  margin-left: 4px;
}}
.overview-cell-window {{
  font-size: 0.7em;
  color: {C_SUB};
  background: #F4F0E4;
  padding: 3px 8px;
...
/* Scope badge for analysis box evidence sections */
.scope-badge {{
  display: inline-block;
  font-size: 0.7em;
  font-weight: 600;
...
def reset_state():
    ...
    for ai in AI_CATALOG:
        ...
        st.session_state[f"ai_manual_{ai['id']}"] = False
    ...
    # Track previous trigger states so we can detect off→on transitions
    # and emit toast notifications + alert banners
    st.session_state.prev_tc01_phase = "normal"
    st.session_state.prev_tc02_phase = "normal"
    st.session_state.prev_tc03_active = False
    # Pending toast queue — items emitted at next UI render and cleared
    st.session_state.pending_toasts = []

    # ===== Closed-loop auto-recovery system =====
    # When triggers fire, the dashboard auto-advances ticks (with sleep+rerun)
    # to demonstrate lever-driven recovery of PXI/EXI back toward baseline.
    # This visualizes the FULL closed loop: signal → trigger → lever → recovery → release.
    st.session_state.recovery_in_progress = False
    st.session_state.recovery_tick_count = 0  # safety counter to prevent infinite loops
...
def get_effective_ai_aal(ai):
    """Effective AAL. Tracks the auto value (from TC state) unless the user
    has manually moved this AI's slider. Pure read — no state mutation."""
    if not st.session_state.get(f"ai_manual_{ai['id']}", False):
        return compute_ai_auto_aal(ai["tier"])
    key = f"ai_aal_slider_{ai['id']}"
    return st.session_state.get(key, compute_ai_auto_aal(ai["tier"]))


def is_ai_manual_override(ai):
    """True only if the user has explicitly moved this AI's slider."""
    return st.session_state.get(f"ai_manual_{ai['id']}", False)


def _mark_ai_manual(manual_key):
    """Slider on_change callback: mark this AI as manually overridden."""
    st.session_state[manual_key] = True


def fleet_avg_aal():
    """Display-only: mean effective AAL across all monitored AIs.
    Mirrors how sub-dimensions roll up to PXI/EXI. Does NOT feed simulation
    logic (which uses the conceptual automation_level via c_AAL())."""
    if not AI_CATALOG:
        return c_AAL()
    return sum(get_effective_ai_aal(ai) for ai in AI_CATALOG) / len(AI_CATALOG)


def fleet_display_if():
    """Display-only: IF shown as the inverse-symmetric mirror of the
    fleet-average AAL — i.e. IF = 1 + (1 - avg_AAL). Keeps the AAL/IF
    inverse-symmetry visible on screen. Does NOT feed simulation logic
    (which uses interaction_mult via c_IF())."""
    return 1.0 + (1.0 - fleet_avg_aal())
...
# Per-AI action vocabulary for the activity timeline (what the AI actually did)
AI_ACTIONS = {
    "AI-001": ("흉부 X-ray 판독", "초음파 영상 분석", "영상 이상 소견 플래그"),
    "AI-002": ("패혈증 위험 점수 산출", "환자 악화 조기 경보", "활력징후 이상 알림"),
    "AI-003": ("약물 상호작용 검토", "알러지 충돌 경고", "용량 조정 권고"),
    "AI-004": ("음성 진료기록 변환", "ICD 코드 자동 부여", "퇴원 요약 초안 생성"),
    "AI-005": ("이상 검사값 자동 플래그", "검사 결과 해석 보조", "위험 수치 알림"),
    "AI-006": ("병상 배정 최적화", "인력 스케줄 조정", "수술실 일정 재배치"),
}


def get_ai_activity_timeline(ai, n_ticks=6):
    """Reconstruct what an AI did over the most recent ticks (deterministic).
    Returns a list of (tick, action, outcome) newest-first. No state stored."""
    s = st.session_state
    actions = AI_ACTIONS.get(ai["id"], ("운영 작업",))
    rows = []
    start = max(0, s.tick - n_ticks + 1)
    for t in range(start, s.tick + 1):
        seed = (t * 1000) + sum(ord(c) for c in ai["id"]) * 7
        rng = np.random.default_rng(seed)
        # effective AAL at that tick is approximated by the current one
        eff_aal = get_effective_ai_aal(ai)
        action = actions[int(rng.integers(0, len(actions)))]
        n_ops = ai["base_activity"] + int(rng.integers(-12, 13))
        esc = int(rng.poisson(0.4 + (1 - eff_aal) * 6))
        err = int(rng.poisson(0.2 + (1 - eff_aal) * 2))
        if err > 0:
            outcome = f"{max(0, n_ops)}건 처리 · 오류 {err}건 검토"
        elif esc > 0:
            outcome = f"{max(0, n_ops)}건 처리 · {esc}건 인간 검토 상향"
        else:
            outcome = f"{max(0, n_ops)}건 자동 처리"
        rows.append((t, action, outcome))
    rows.reverse()
    return rows
...
# ===== Signal dynamics =====
def generate_signal():
    """Each click monotonically pushes PXI and EXI toward their trigger thresholds.
    With baseline INIT_PXI=75, INIT_EXI=70 and decrements of ~1.6 / ~1.5 per click,
    the 4th click reliably crosses both TC-01 (PXI<70) and TC-02 (EXI<65) thresholds.
    Small noise (±0.3) preserves visual variation without breaking determinism."""
    s = st.session_state
    s.tick += 1
    # Deterministic monotonic drop with small noise — guarantees demo behavior
    # PXI 75 → ~67.5 (4 clicks): 100% below TC-01 threshold (70)
    # EXI 70 → ~63.0 (4 clicks): 100% below TC-02 threshold (65)
    s.PXI = float(np.clip(s.PXI - 1.8 + np.random.normal(0, 0.2), 35, 100))
    s.EXI = float(np.clip(s.EXI - 1.7 + np.random.normal(0, 0.2), 35, 100))
    apply_tc_rules()
    ...
    # Note: trigger fires LEAVE the system in engaged state — recovery is NOT
    # auto-triggered here. Operator/presenter must explicitly click
    # "▶ Run closed-loop recovery" to demonstrate the recovery phase.
    # This gives time to show Decision Support panel + narrate lever changes.


def force_exi_shock(magnitude=15):
    """Demo-only: reproduce S4 scenario by inducing cross-domain divergence.
    EXI drops sharply (-magnitude), PXI is left unchanged. From baseline state
    (PXI≈75, EXI≈70), the resulting gap (|75 − 55| = 20) reliably fires TC-03.
    The auto-recovery cycle that follows demonstrates closed-loop convergence
    back to baseline through TC-02 levers (HOI ↑, TR ↑)."""
    s = st.session_state
    s.tick += 1
    # PXI: unchanged (left at whatever its current value is)
    # EXI: sharp drop (employee experience shock)
    ...
    for sub in PXI_SUBS + EXI_SUBS:
        _hist[sub['id']] = round(st.session_state.get(f"sub_{sub['id']}", 0), 2)
    s.history.append(_hist)
    # Note: shock LEAVES the system in engaged + TC-03 state. Operator/presenter
    # must explicitly click "▶ Run closed-loop recovery" to start animation.


def force_pxi_shock(magnitude=21):
    """Demo-only: pandemic-level patient-experience shock. PXI drops sharply
    (-magnitude). The figure is set slightly beyond the COVID-era reported
    decline (~-20) because the pandemic occurred WITHOUT AI governance in place;
    the deeper drop represents an unmitigated patient-experience collapse. EXI
    is left unchanged. From baseline (PXI≈75) this drives PXI to ≈54 — below
    TH_PXI (70), firing TC-01 — while the PXI/EXI gap widens to ≈16 (> 15),
    also firing the cross-domain divergence trigger TC-03. Mirror of the
    workforce-side EXI shock, from the patient side."""
    s = st.session_state
    s.tick += 1
    # EXI: unchanged
    # PXI: sharp drop (pandemic-level, unmitigated patient experience collapse)
    s.PXI = float(np.clip(s.PXI - magnitude, 35, 100))
    apply_tc_rules()
    update_sub_dimensions()
    _hist = {
        "tick":  s.tick,
        ...
        "TC-02": s.tc02_phase,
        "TC-03": "active" if s.tc03_active else "off",
        "action": f"PXI SHOCK -{magnitude} (pandemic-level, demo) | {last_action()}",
    }
    for sub in PXI_SUBS + EXI_SUBS:
        _hist[sub['id']] = round(st.session_state.get(f"sub_{sub['id']}", 0), 2)
    s.history.append(_hist)
    ...
    # ===== Detect off→on transitions and queue toast notifications =====
    # TC-01: normal/stabilizing → engaged
    if s.tc01_phase == "engaged" and s.get("prev_tc01_phase", "normal") != "engaged":
        s.pending_toasts.append({
            "msg": f"TC-01 ENGAGED · PXI {s.PXI:.1f} below threshold 70",
            "icon": "🚨"
        })
    # TC-02: normal/stabilizing → engaged
    if s.tc02_phase == "engaged" and s.get("prev_tc02_phase", "normal") != "engaged":
        s.pending_toasts.append({
            "msg": f"TC-02 ENGAGED · EXI {s.EXI:.1f} below threshold 65",
            "icon": "🚨"
        })
    # TC-03: off → active (most urgent)
    if s.tc03_active and not s.get("prev_tc03_active", False):
        s.pending_toasts.append({
            "msg": f"TC-03 URGENT · Cross-domain gap |PXI−EXI| = {abs(s.PXI - s.EXI):.1f} exceeds 15",
            "icon": "🔥"
        })

    # Update previous state for next transition detection
    s.prev_tc01_phase = s.tc01_phase
    s.prev_tc02_phase = s.tc02_phase
    s.prev_tc03_active = s.tc03_active


def emit_pending_toasts():
    """Emit queued toast notifications and clear the queue.
    Called once per UI render at the top of the page."""
    s = st.session_state
    pending = s.get("pending_toasts", [])
...
def auto_recovery_tick():
    """Apply ONE tick of lever-driven closed-loop recovery.
    This is the missing half of the closed loop:
      Signal → Trigger → Lever engagement → [RECOVERY HERE] → Trigger release → baseline.

    Recovery dynamics (per tick) — each engaged lever pushes the corresponding
    metric back toward baseline:
      PXI recovery = (1−AAL)·5.0  +  (IF−1)·4.0  +  (HOI−0.25)·1.5  +  (TR−0.5)·1.0
        ├ AAL ↓ : AI restraint reduces risky decisions → patient feels safer
        ├ IF ↑  : stronger information flow (explanation, disclosure) → patient trust recovers
        ├ HOI ↑ : staff feel supported → better care delivered to patients
        └ TR ↑  : trust recovery → improved patient communication

      EXI recovery = (HOI−0.25)·2.0  +  (TR−0.5)·2.0  +  (1−AAL)·1.0  +  (IF−1)·0.5
        ├ HOI ↑ : staff feel oversight is in place → workload anxiety drops
        ├ TR ↑  : trust recovery package → engagement returns
        ├ AAL ↓ : less risky AI decisions → staff confidence
        └ IF ↑  : transparent information flow reduces staff uncertainty in workflow

    With small Gaussian noise (±0.3) for visual variation. apply_tc_rules()
    handles the phase transitions: engaged → stabilizing → normal,
    plus the gradual lever release (RELEASE_AAL/IF/HOI/TR per tick).
    """
    s = st.session_state
    s.tick += 1
    s.recovery_tick_count += 1

    # Compute lever-driven recovery forces
    lever_force_pxi = 0.0
    lever_force_exi = 0.0

    # PXI recovery from TC-01 levers (direct)
    if c_AAL() < 1.0:
        lever_force_pxi += (1.0 - c_AAL()) * 5.0
    if c_IF() > 1.0:
        lever_force_pxi += (c_IF() - 1.0) * 4.0
    # PXI recovery from TC-02 levers (indirect via staff effect)
    if c_HOI() > 0.25:
        lever_force_pxi += (c_HOI() - 0.25) * 1.5
    if c_TR() > 0.5:
        lever_force_pxi += (c_TR() - 0.5) * 1.0

    # EXI recovery from TC-02 levers (direct)
    if c_HOI() > 0.25:
        lever_force_exi += (c_HOI() - 0.25) * 2.0
    if c_TR() > 0.5:
        lever_force_exi += (c_TR() - 0.5) * 2.0
    # EXI recovery from TC-01 levers (indirect via reduced risk)
    if c_AAL() < 1.0:
        lever_force_exi += (1.0 - c_AAL()) * 1.0
    if c_IF() > 1.0:
        lever_force_exi += (c_IF() - 1.0) * 0.5

    # Saturation: cap recovery so values don't overshoot baseline.
    # When at or above baseline, recovery force is zero (no need to push higher).
    # When below baseline, recovery force is min(lever_force, deficit + 0.5) — slows near baseline.
    pxi_deficit = INIT_PXI - s.PXI
    if pxi_deficit > 0:
        pxi_recovery = min(lever_force_pxi, pxi_deficit + 0.5)
    else:
        pxi_recovery = 0.0  # at/above baseline — no further recovery needed

    exi_deficit = INIT_EXI - s.EXI
    if exi_deficit > 0:
        exi_recovery = min(lever_force_exi, exi_deficit + 0.5)
    else:
        exi_recovery = 0.0

    # Apply with small noise for visual variation
    s.PXI = float(np.clip(s.PXI + pxi_recovery + np.random.normal(0, 0.3), 35, 100))
    s.EXI = float(np.clip(s.EXI + exi_recovery + np.random.normal(0, 0.3), 35, 100))

    apply_tc_rules()  # may transition engaged → stabilizing → normal
    update_sub_dimensions()

    # Log to history (marked as auto-recovery for transparency)
    _hist = {
        "tick":  s.tick,
        "time":  datetime.now().strftime("%H:%M:%S"),
        "PXI":   round(s.PXI, 2), "EXI": round(s.EXI, 2),
        ...
        "TC-02": s.tc02_phase,
        "TC-03": "active" if s.tc03_active else "off",
        "action": f"🔄 Auto-recovery | {last_action()}",
    }
    for sub in PXI_SUBS + EXI_SUBS:
        _hist[sub['id']] = round(st.session_state.get(f"sub_{sub['id']}", 0), 2)
    s.history.append(_hist)
...
def check_recovery_complete():
    """Returns True if all triggers have released (recovery cycle complete)."""
    s = st.session_state
    return (s.tc01_phase == "normal" and
            s.tc02_phase == "normal" and
            not s.tc03_active)


def render_alert_banner():
    """Render a persistent alert banner at the top when any trigger is active.
    Severity hierarchy:
      - TC-03 active        → 🔥 URGENT (st.error, most prominent)
      - TC-01 AND TC-02     → 🚨 IMMEDIATE (st.error)
      - TC-01 or TC-02 only → ⚠ WARNING (st.warning)
      - none                → no banner
    This is the dashboard's PUSH mechanism — operator does not need to
    actively monitor; the system alerts when intervention is required."""
    s = st.session_state

    tc01_eng = (s.tc01_phase == "engaged")
    tc02_eng = (s.tc02_phase == "engaged")
    tc03_act = s.tc03_active

    if not (tc01_eng or tc02_eng or tc03_act):
        return  # Normal operation — no banner

    # Most urgent: TC-03 cross-domain divergence
    if tc03_act:
        gap = abs(s.PXI - s.EXI)
        st.error(
            f"🔥 **URGENT — TC-03 Cross-Domain Divergence Active**  ·  "
            f"|PXI − EXI| = {gap:.1f}  (threshold 15)  ·  "
            f"PXI {s.PXI:.1f}  ·  EXI {s.EXI:.1f}  ·  "
            f"All four levers in recovery state — immediate review required."
        )
        return

    # High: both TC-01 and TC-02
    if tc01_eng and tc02_eng:
        st.error(
            f"🚨 **IMMEDIATE ATTENTION — TC-01 + TC-02 Both Engaged**  ·  "
            f"PXI {s.PXI:.1f} < 70  ·  EXI {s.EXI:.1f} < 65  ·  "
            f"Auto-adjusted levers: AAL ↓ 0.80  ·  HOI ↑ 1.00  ·  IF ↑ 1.20  ·  TR ↑ 1.00"
        )
        return
    ...
    parts = []
    if tc01_eng:
        parts.append(f"TC-01 active (PXI {s.PXI:.1f} below 70)")
    if tc02_eng:
        parts.append(f"TC-02 active (EXI {s.EXI:.1f} below 65)")
    st.warning(
        f"⚠ **Trigger Engaged** · " + " · ".join(parts) +
        " · Levers auto-adjusted. Review Decision Support below."
    )
...
def last_action():
    ...
    if s.tc03_active: parts.append("TC-03 ENGAGED")
...
def interpretation():
    ...
    if s.tc03_active:
        ...
        parts.append(
            ...
            "Cross-rebalancing toward the weaker domain (HRO cross-domain decoupling, Reason 1997)."
        )
    return "  \n\n".join(parts)
...
def generate_overview_analysis():
    ...
    if tc01 == "normal" and tc02 == "normal" and not tc03:
        ...
        op_en = (
            ...
            "clinician verification. Information flow strengthened by 20%. All AI "
        ...
        op_kr = (
            ...
            "필수입니다. 정보 흐름이 20% 강화되었습니다. 모든 AI 의사결정에 근거가 자동 "
        ...
        op_en = (
            ...
            "Patient-facing AI suggestions are screened by nursing staff. Information "
            "flow strengthened by 20% (IF: 1.00 → 1.20 — more explanation, disclosure, "
            "and rationale sharing). The hospital is autonomously protecting patient "
        ...
        op_kr = (
            ...
            "대상 AI 추천은 간호 인력 검토를 거칩니다. 정보 흐름이 20% 강화되었습니다"
            "(IF: 1.00 → 1.20, 설명·공개·근거 공유 증가). 병원이 외부 지침 없이 "
        ...
        op_en = (
            ...
            "domain is significantly better than the other. TC-03 fires cross-rebalancing "
            "and notifies the governance committee for review."
        )
        op_kr = (
            "동시 트리거 활성 없이 도메인 간 격차(|PXI − EXI| > 15)가 감지되었습니다. "
            "이는 한 도메인이 다른 도메인보다 유의미하게 양호한 비대칭 경험 신호를 "
            "의미합니다. TC-03이 교차 재균형을 작동시키고 거버넌스 위원회 검토를 위해 통지합니다."
        )
    ...
    if abs(if_val - 1.0) < 0.01:
        if_en = "Routine information flow: standard transparency and explanation sharing maintained."
        if_kr = "평시 정보 흐름: 표준 수준의 투명성·설명 공유 유지."
    elif if_val >= 1.10:
        pct = round((if_val - 1) * 100)
        if_en = f"Strengthened information flow: {if_val:.2f}× baseline (+{pct}% more explanation, disclosure, and rationale sharing)."
        if_kr = f"정보 흐름 강화: baseline의 {if_val:.2f}배(설명·공개·근거 공유 +{pct}%)."
    else:
        if_en = f"Slightly strengthened information flow: {if_val:.2f}× baseline."
        if_kr = f"정보 흐름 소폭 강화: baseline의 {if_val:.2f}배."
...
def _build_decision_support(s):
    ...
    if tc01 == "engaged":
        ...
        items.append({
            "priority": "TODAY",
            "en": "Brief nursing staff on the strengthened information-flow protocol (IF +20% — more explanation, disclosure, and rationale sharing with patients).",
            "kr": "간호 인력에 정보 흐름 강화 프로토콜 안내 (IF +20% — 환자 대상 설명·공개·근거 공유 증가)."
        })
    if tc02 == "engaged":
        items.append({
            "priority": "IMMEDIATE",
...
with st.sidebar:
    ...
    st.caption("Inject an asymmetric experience shock · 한쪽 경험만 급락시켜 cross-domain 발산(TC-03) 시연")
    _shock_pxi_col, _shock_exi_col = st.columns(2)
    with _shock_pxi_col:
        if st.button("PXI shock", use_container_width=True,
                     help="환자경험만 팬데믹 수준으로 급락 (-21, 직원경험 그대로) → TC-01 + TC-03 발화"):
            force_pxi_shock()
    with _shock_exi_col:
        if st.button("EXI shock", use_container_width=True,
                     help="직원경험만 급락 (-15, 환자경험 그대로) → TC-02 + TC-03 발화"):
            force_exi_shock()

    # ===== Closed-loop recovery trigger =====
    # Triggers explicit auto-recovery animation. Used in demo AFTER showing
    # Decision Support panel — once narration of engaged state is complete,
    # presenter clicks this to demonstrate the lever-driven recovery phase.
    st.caption("Start auto-recovery animation (closed-loop demo)")
    _any_trigger = (st.session_state.tc01_phase == "engaged"
                    or st.session_state.tc02_phase == "engaged"
                    or st.session_state.tc03_active)
    _recovery_running = st.session_state.get("recovery_in_progress", False)
    if st.button("▶ Run closed-loop recovery",
                 use_container_width=True,
                 disabled=(not _any_trigger or _recovery_running),
                 help=("Click after triggers fire to start auto-recovery. "
                       "Disabled when no triggers active or recovery already running.")):
        st.session_state.recovery_in_progress = True
        st.session_state.recovery_tick_count = 0
        st.rerun()
    if _recovery_running:
        st.caption(f"🔄 Recovery in progress · tick {st.session_state.recovery_tick_count}")
    st.markdown("---")
...
| **TC-03** | \|PXI − EXI\| > 15 | Cross rebalance — AAL/IF/HOI/TR rebalanced toward the lower domain |
        """)
        st.caption(
            ...
            "(Lee & See, 2004). TC-03 is a cross-domain trigger: when one experience "
            "domain falls significantly lower than the other (|PXI − EXI| > 15), it "
            "fires cross-rebalancing toward the weaker domain — interpreted via HRO "
            "cross-domain decoupling (Reason 1997)."
        )
...
| --- | --- | --- |
| TH_PXI | 70 | Operating baseline (75) − 1 MCID (5pt) · HCAHPS poor-zone boundary |
| TH_EXI | 65 | HRO burnout-warning cutoff (Reason 1997) |
| TH_GAP | 15 | Normal gap 4–6 (empirical, S1–S3) → 3 MCID divergence · HRO cross-domain decoupling (Reason 1997) |
...
# ===== Push notifications + persistent alert banner =====
# Toast notifications fire when triggers transition off→on (one-time alerts).
# Banner persists at top of screen whenever any trigger is active.
# This is the dashboard's PUSH mechanism — operators do not need to
# actively monitor; the system surfaces alerts when intervention is needed.
emit_pending_toasts()
render_alert_banner()
...
with tab_overview:
    ...
    bil_caption(
        "Multi-temporal synthesis across all four HCEE layers · "
        "real-time state + tab summaries + decision support",
        "HCEE 4개 layer의 multi-temporal 종합 · "
        "실시간 상태 + 탭 요약 + 의사결정 지원"
    )
    ...
    # ============================================================
    st.markdown(
        "<div class='overview-sec-header'>"
        "<span class='overview-sec-num'>①</span>"
        "<span class='overview-sec-title-en'>Current Real-time State</span>"
        "<span class='overview-sec-title-kr'>· 현재 실시간 상태</span>"
        "<div class='overview-sec-caption'>"
        "Instantaneous values from the operational layer — "
        "this is a snapshot, not a synthesis"
        "<br/>운영 레이어의 즉시 값 — 종합이 아닌 스냅샷"
        "</div>"
        "</div>",
        unsafe_allow_html=True
    )
    ...
    om = st.columns(6)
    om[0].metric("PXI", f"{s.PXI:.2f}", f"{s.PXI - TH_PXI:+.1f} vs TC-01")
    om[1].metric("EXI", f"{s.EXI:.2f}", f"{s.EXI - TH_EXI:+.1f} vs TC-02")
    _avg_aal_disp = fleet_avg_aal()
    om[2].metric("AAL", f"{_avg_aal_disp:.2f}",
                 f"{_avg_aal_disp - CONCEPT_BASELINES['AAL']:+.2f}" if abs(_avg_aal_disp - CONCEPT_BASELINES['AAL']) > 0.005 else None)
    om[3].metric("HOI", f"{c_HOI():.2f}",
                 f"{c_HOI() - CONCEPT_BASELINES['HOI']:+.2f}" if c_HOI() != CONCEPT_BASELINES['HOI'] else None)
    om[4].metric("IF",  f"{fleet_display_if():.2f}",
                 f"{fleet_display_if() - CONCEPT_BASELINES['IF']:+.2f}" if abs(fleet_display_if() - CONCEPT_BASELINES['IF']) > 0.005 else None)
    ...
    # ============================================================
    # ===== ② Multi-Tab Synthesis (2x2 grid) =====
    # ============================================================
    st.markdown(
        "<div class='overview-sec-header'>"
        "<span class='overview-sec-num'>②</span>"
        "<span class='overview-sec-title-en'>Multi-Tab Synthesis</span>"
        "<span class='overview-sec-title-kr'>· 4개 레이어 종합</span>"
        "<div class='overview-sec-caption'>"
        "Each cell aggregates over its <b>natural temporal scope</b> — "
        "this is HCEE's multi-temporal architecture in action "
        "(Folke et al. 2005, Cash et al. 2003)."
        "<br/>각 셀은 <b>자연 시간 척도</b>로 종합됨 — HCEE multi-temporal "
        "architecture의 작동 방식"
        "</div>"
        "</div>",
        unsafe_allow_html=True
    ...
    # === Cell 1: Daily Governance Loop ===
    with r1c1:
        st.markdown(
            "<div class='overview-cell-header overview-cell-header-daily'>"
            "<div>"
            "<span class='overview-cell-icon'>🔄</span>"
            "<span class='overview-cell-title-en'>Daily Governance Loop</span>"
            "<span class='overview-cell-title-kr'>거버넌스 루프</span>"
            "</div>"
            "<span class='overview-cell-window'>⏱ Recent 30 ticks · 운영 윈도우</span>"
            "</div>",
            unsafe_allow_html=True
        )
        # Mini PXI/EXI chart
        if s.history:
            ...
            fig_p.update_layout(height=170, margin=dict(l=10, r=10, t=6, b=20),
                                xaxis_title="", yaxis_title="",
                                yaxis_range=[40, 95],
                                paper_bgcolor=C_BG, plot_bgcolor="#FFFFFF",
                                font=dict(color=C_TITLE, size=10),
                                legend=dict(orientation="h", y=1.16, x=0.0,
                                            font=dict(size=10)))
        ...
        # Insight summary
        _phases = []
        if s.tc01_phase == "engaged": _phases.append("TC-01 engaged")
        elif s.tc01_phase == "stabilizing": _phases.append("TC-01 recovering")
        if s.tc02_phase == "engaged": _phases.append("TC-02 engaged")
        elif s.tc02_phase == "stabilizing": _phases.append("TC-02 recovering")
        if s.tc03_active: _phases.append("TC-03 active")
        _phase_text = ", ".join(_phases) if _phases else "All triggers normal"
        st.markdown(
            f"<div class='overview-cell-insight'>"
            f"<b>{_phase_text}</b> · {s.tc01_engagements + s.tc02_engagements} total engagements "
            f"in window (TC-01: {s.tc01_engagements}, TC-02: {s.tc02_engagements})"
            f"</div>"
            f"<div class='overview-cell-footer'>"
            "→ See \"Daily Governance Loop\" tab for full Event Timeline + sub-dimensions"
            "</div>",
            unsafe_allow_html=True
        )

    # === Cell 2: AI Monitoring ===
    with r1c2:
        st.markdown(
            "<div class='overview-cell-header overview-cell-header-ai'>"
            "<div>"
            "<span class='overview-cell-icon'>🤖</span>"
            "<span class='overview-cell-title-en'>AI Monitoring</span>"
            "<span class='overview-cell-title-kr'>AI 모니터링</span>"
            "</div>"
            "<span class='overview-cell-window'>⏱ Current state + 24h rolling</span>"
            "</div>",
            unsafe_allow_html=True
        )
        # Compute AI status counts
        _ai_status_count = {"green": 0, "amber": 0, "amber-light": 0, "red": 0, "violet": 0}
        _ai_total_activity = 0
        _ai_total_escalations = 0
        _ai_total_errors = 0
        for _ai in AI_CATALOG:
            _, _, _color, _ = get_ai_status(_ai)
            _stats = get_ai_24h_stats(_ai)
            _ai_status_count[_color] = _ai_status_count.get(_color, 0) + 1
            _ai_total_activity += _stats["activity"]
            _ai_total_escalations += _stats["escalations"]
            _ai_total_errors += _stats["errors"]
        _ai_active = _ai_status_count["green"]
        _ai_restricted = _ai_status_count["amber"] + _ai_status_count["amber-light"]
        _ai_review = _ai_status_count["red"]
        _ai_override = _ai_status_count["violet"]

        # AI status dots visual
        _dots_html = ""
        for _ai in AI_CATALOG:
            _, _, _color, _sym = get_ai_status(_ai)
            _color_map = {"green": C_PXI, "amber": C_STAB, "amber-light": "#D4B670",
                          "red": C_ON, "violet": "#7B4CBF"}
            _ai_title = f"{_ai['name']} ({_color})"
            _ai_num = _ai['id'].split('-')[1][-1]
            _bg = _color_map.get(_color, '#BFBFBF')
            _dots_html += (
                f"<span title='{_ai_title}' "
                f"style='display:inline-block; width:18px; height:18px; "
                f"background:{_bg}; "
                f"border-radius:50%; margin-right:6px; "
                f"text-align:center; color:white; font-size:0.8em; line-height:18px;'>"
                f"{_ai_num}</span>"
            )
        st.markdown(
            f"<div style='padding:8px 4px;'>"
            f"<div style='font-size:0.8em; color:{C_SUB}; margin-bottom:4px;'>"
            f"6 AIs (numbered 1-7, with 6 omitted):</div>"
        ...
        _ai_cols[0].metric("Active", _ai_active)
        _ai_cols[1].metric("Restricted", _ai_restricted)
        _ai_cols[2].metric("Override", _ai_override)
        _ai_cols[3].metric("Total", len(AI_CATALOG))

        st.markdown(
            f"<div class='overview-cell-insight'>"
            f"<b>24h totals:</b> {_ai_total_activity:,} operations · "
            f"{_ai_total_escalations} escalations · {_ai_total_errors} errors"
            f"</div>"
            f"<div class='overview-cell-footer'>"
            "→ See \"Real-time AI\" tab for per-AI cards + AAL sliders"
            "</div>",
            unsafe_allow_html=True
        )
    ...
    # === Cell 3: Experience Drivers (Sub-Dimension) ===
    with r2c1:
        st.markdown(
            "<div class='overview-cell-header overview-cell-header-drivers'>"
            "<div>"
            "<span class='overview-cell-icon'>🔍</span>"
            "<span class='overview-cell-title-en'>Experience Drivers</span>"
            "<span class='overview-cell-title-kr'>경험 driver 차원</span>"
            "</div>"
            "<span class='overview-cell-window'>⏱ Current snapshot · 즉시</span>"
            "</div>",
            unsafe_allow_html=True
        )
        # Top 2 lowest in each domain
        _pxi_sorted = sorted(
        ...
        _drv_html = "<div style='font-size:0.84em; line-height:1.5; padding:4px;'>"
        _drv_html += f"<div style='color:{C_PXI}; font-weight:700; margin-bottom:3px;'>PXI · 환자경험</div>"
        for i, (sub, val) in enumerate(_pxi_sorted):
            ...
            _drv_html += (
                ...
                f"<span style='color:{C_ON}; font-size:0.85em; font-weight:600;'>{_flag}</span>"
                f"</div>"
            )
        _drv_html += f"<div style='color:{C_EXI}; font-weight:700; margin-top:6px; margin-bottom:3px;'>EXI · 직원경험</div>"
        ...
        _ai_related_low = (
            ("pxi_ai_trust" in [sub['id'] for sub, _ in _pxi_sorted]) and
            (("exi_ai_trust" in [sub['id'] for sub, _ in _exi_sorted]) or
             ("exi_usability" in [sub['id'] for sub, _ in _exi_sorted]))
        )
        if _ai_related_low:
            _pattern_text = (
                "Cross-domain pattern: AI-related dimensions weak on both sides "
                "→ EMR-AI integration friction suspected"
            )
        else:
            _pattern_text = (
                "No cross-domain AI pattern detected; drivers are domain-independent"
            )
        st.markdown(
            f"<div class='overview-cell-insight'>{_pattern_text}</div>"
            f"<div class='overview-cell-footer'>"
            "→ See \"Daily Governance Loop\" tab for full 12-dim breakdown + time-series"
            "</div>",
            unsafe_allow_html=True
        )

    # === Cell 4: Adaptive Learning ===
    with r2c2:
        st.markdown(
            "<div class='overview-cell-header overview-cell-header-learning'>"
            "<div>"
            "<span class='overview-cell-icon'>📈</span>"
            "<span class='overview-cell-title-en'>Adaptive Learning</span>"
            "<span class='overview-cell-title-kr'>적응 학습</span>"
            "</div>"
            "<span class='overview-cell-window'>⏱ All-time · since deployment</span>"
            "</div>",
            unsafe_allow_html=True
        )
        # Cumulative metrics
        _al_cols = st.columns(4)
        _al_cols[0].metric("TC-01", s.tc01_engagements)
        _al_cols[1].metric("TC-02", s.tc02_engagements)
        _al_cols[2].metric("TC-03", s.tc03_engagements)
        _al_cols[3].metric("Ticks", s.tick)
        # Active ratio
        _tot = max(1, len(s.history))
        _eng_ticks = sum(
            1 for h in s.history
            if h["TC-01"] == "engaged" or h["TC-02"] == "engaged"
            or h["TC-03"] == "active"
        ) if s.history else 0
        _eng_ratio = round(_eng_ticks / _tot * 100, 1) if s.history else 0
        # Mini activation pattern (last 30 ticks)
        if s.history and len(s.history) >= 5:
            df_l = pd.DataFrame(s.history[-30:])
            df_l["any_active"] = (
                (df_l["TC-01"] == "engaged").astype(int) +
                (df_l["TC-02"] == "engaged").astype(int) +
                (df_l["TC-03"] == "active").astype(int)
            )
            fig_l = go.Figure()
            fig_l.add_trace(go.Bar(x=df_l["tick"], y=df_l["any_active"],
                                   marker_color="#4A6FA5", showlegend=False))
            fig_l.update_layout(height=100, margin=dict(l=10, r=10, t=6, b=18),
                                xaxis_title="", yaxis_title="",
                                yaxis_range=[0, 3.2], yaxis_showticklabels=False,
                                paper_bgcolor=C_BG, plot_bgcolor="#FFFFFF",
                                font=dict(color=C_TITLE, size=9))
            st.plotly_chart(fig_l, use_container_width=True)
        # Pattern insight
        _total_eng = s.tc01_engagements + s.tc02_engagements + s.tc03_engagements
        if _total_eng == 0 and s.tick >= 20:
            _pattern = "No activation patterns yet — system over-stable"
        elif s.tc01_engagements >= 3:
            _pattern = f"⚠ TC-01 pattern detected ({s.tc01_engagements}× fires) — policy review"
        elif s.tc02_engagements >= 3:
            _pattern = f"⚠ TC-02 pattern detected ({s.tc02_engagements}× fires) — workload review"
        elif s.tc03_engagements >= 2:
            _pattern = f"⚠ TC-03 pattern detected ({s.tc03_engagements}× active) — recalibration"
        else:
            _pattern = f"Active tick ratio: {_eng_ratio}% · monitoring nominal"
        st.markdown(
            f"<div class='overview-cell-insight'>{_pattern}</div>"
            f"<div class='overview-cell-footer'>"
            "→ See \"Adaptive Learning\" tab for activation history + policy recommendations"
            "</div>",
            unsafe_allow_html=True
        )
    ...
    st.markdown(
        "<div class='overview-sec-header'>"
        "<span class='overview-sec-num'>③</span>"
        "<span class='overview-sec-title-en'>Decision Support &amp; Analysis</span>"
        "<span class='overview-sec-title-kr'>· 의사결정 지원 및 분석</span>"
        "<div class='overview-sec-caption'>"
        "Priority-tagged action items + evidence base · click to expand if collapsed"
        "<br/>우선순위 별 조치 항목 + 근거 자료 · 접혀있으면 클릭하여 펼침"
        "</div>"
        "</div>",
        unsafe_allow_html=True
    )

    # Analysis box (collapsible, with Decision Support at top)
    ...
    _analysis_html = (
        ...
        # ===== 4 evidence sections (with time-scope badges) =====
        "<div class='analysis-section'>"
        "<div class='analysis-section-title'>"
        "① Operational Mode · 현재 운영 모드"
        "<span class='scope-badge'>⏱ Current snapshot</span>"
        "<br/>"
        ...
        "<span class='scope-badge'>⏱ Current lever state</span>"
        "<br/>"
        "<span style='font-weight:400; font-size:0.85em; color:#7E776B;'>"
        ...
        "<span class='scope-badge'>⏱ Current + cumulative</span>"
        "<br/>"
        "<span style='font-weight:400; font-size:0.85em; color:#7E776B;'>"
        ...
        "<div class='analysis-section-title'>"
        "④ Trajectory · 추이"
        "<span class='scope-badge'>⏱ Last 5 ticks</span>"
        "<br/>"
        "<span style='font-weight:400; font-size:0.85em; color:#7E776B;'>"
...
with tab_loop:
    ...
    m = st.columns(6)
    m[0].metric("PXI", f"{s.PXI:.2f}", f"{s.PXI - TH_PXI:+.1f} vs TC-01")
    m[1].metric("EXI", f"{s.EXI:.2f}", f"{s.EXI - TH_EXI:+.1f} vs TC-02")
    m[2].metric("AAL", f"{fleet_avg_aal():.2f}",
                f"{fleet_avg_aal() - CONCEPT_BASELINES['AAL']:+.2f}" if abs(fleet_avg_aal() - CONCEPT_BASELINES['AAL']) > 0.005 else None)
    m[3].metric("HOI", f"{c_HOI():.2f}",
                f"{c_HOI() - CONCEPT_BASELINES['HOI']:+.2f}" if c_HOI() != CONCEPT_BASELINES['HOI'] else None)
    m[4].metric("IF",  f"{fleet_display_if():.2f}",
                f"{fleet_display_if() - CONCEPT_BASELINES['IF']:+.2f}" if abs(fleet_display_if() - CONCEPT_BASELINES['IF']) > 0.005 else None)
    ...
    def _render_sub_block(sub_list, composite_value, composite_label, threshold,
        ...
        for sub, val in rows:
            ...
            rows_html += (
                ...
                f"<span class='subdim-name-kr' style='color:{C_SUB}; font-size:0.78em;'>"
                f"↔ {' · '.join(sub.get('linked_ai', []))}</span>"
                f"</div>"
...
with tab_ai:
    ...
    # Fleet-average effective AAL (mirrors how sub-dimensions roll up to PXI/EXI)
    _avg_aal = (sum(_d[6] for _d in _ai_details) / len(_ai_details)) if _ai_details else 1.0
    ...
    _summary_html = (
        ...
        "<br/>"
        f"Fleet-average effective AAL · 6개 AI 평균 자율성: "
        f"<strong style='color:#3F4A35;'>{_avg_aal:.2f}</strong>"
        "</div>"
    )
    ...
    for _ai, _st_en, _st_kr, _color, _sym, _stats, _eff_aal in _ai_details:
        ...
        _manual_key = f"ai_manual_{_ai['id']}"
        _is_manual = st.session_state.get(_manual_key, False)
        # Keep slider synced to auto value when not manually overridden, so the
        # widget reflects automatic governance adjustment. Set BEFORE the slider
        # widget is instantiated below — safe.
        if not _is_manual:
            st.session_state[_slider_key] = _auto_aal
        elif _slider_key not in st.session_state:
            st.session_state[_slider_key] = _auto_aal
        _override_note = ""
        if _is_manual:
            _override_note = (
        ...
        # Activity timeline — what this AI did over recent ticks
        with st.expander(f"활동 이력 · Activity log — {_ai['id']}"):
            _timeline = get_ai_activity_timeline(_ai)
            if _timeline:
                _tl_html = "<div style='font-size:0.85em; line-height:1.6;'>"
                for _t, _act, _out in _timeline:
                    _tl_html += (
                        f"<div style='padding:3px 0; border-bottom:1px solid #EFE9DC;'>"
                        f"<strong>tick {_t}</strong> · {_act} "
                        f"<span style='color:{C_SUB};'>— {_out}</span></div>"
                    )
                _tl_html += "</div>"
                st.markdown(_tl_html, unsafe_allow_html=True)
            else:
                st.caption("Run the simulation to populate activity. · 시뮬레이션 실행 시 활동이 기록됩니다.")
...
# ===== Closed-loop auto-recovery =====
# After Generate signal triggers fire OR after EXI shock,
# the dashboard advances ticks automatically (one per 0.7 sec) to demonstrate
# the lever-driven recovery — the second half of the closed loop.
# Stops when all triggers release OR after MAX_RECOVERY_TICKS (safety).
MAX_RECOVERY_TICKS = 20
if st.session_state.get("recovery_in_progress", False):
    if check_recovery_complete():
        # All triggers have released — recovery cycle complete
        st.session_state.recovery_in_progress = False
        st.session_state.recovery_tick_count = 0
        st.toast("✅ Closed-loop recovery complete · all triggers released",
                 icon="✅")
    elif st.session_state.recovery_tick_count >= MAX_RECOVERY_TICKS:
        # Safety: stop after max ticks even if not fully recovered
        st.session_state.recovery_in_progress = False
        st.session_state.recovery_tick_count = 0
        st.toast(f"⏱ Recovery cycle reached max {MAX_RECOVERY_TICKS} ticks",
                 icon="⏱")
    else:
        # Apply one recovery tick and rerun for animation
        auto_recovery_tick()
        time.sleep(0.7)
        st.rerun()
...
# ===== Auto-run loop =====
# In Auto-run mode the closed loop is fully autonomous:
#  - if recovery is already running, the recovery block above handles it
#    (so we do NOT push the signal further down here)
#  - else if any trigger is active, auto-start recovery (no manual button)
#  - else (normal state) keep generating signal to drive the demo
if auto_run:
    _auto_trigger_active = (st.session_state.tc01_phase == "engaged"
                            or st.session_state.tc02_phase == "engaged"
                            or st.session_state.tc03_active)
    if st.session_state.get("recovery_in_progress", False):
        # recovery loop above is driving the reruns; nothing to do here
        pass
    elif _auto_trigger_active:
        # triggers fired → automatically enter closed-loop recovery
        st.session_state.recovery_in_progress = True
        st.session_state.recovery_tick_count = 0
        st.rerun()
    else:
