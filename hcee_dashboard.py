...
def reset_state():
    ...
    for ai in AI_CATALOG:
        st.session_state[f"ai_aal_slider_{ai['id']}"] = compute_ai_auto_aal(ai["tier"])
...
def get_effective_ai_aal(ai):
    """Effective AAL. If the user has not manually moved this AI's slider,
    it tracks the auto value (from TC state). Once manually moved, the
    slider value is the single source of truth until 'Auto' resync.
    Pure read — does not mutate session_state."""
...
with tab_ai:
    ...
    for _ai, _st_en, _st_kr, _color, _sym, _stats, _eff_aal in _ai_details:
        ...
        # When not manually overridden, keep the slider in sync with the auto
        # value so the widget reflects automatic governance adjustment.
        # (Set before the slider widget is instantiated below — safe.)
        if not _is_manual:
