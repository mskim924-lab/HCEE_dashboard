...
def force_exi_shock(magnitude=15):
    ...
    EXI drops sharply (-magnitude), while PXI gets a small recovery boost (+5)
    to simulate patient-side resilience. This guarantees |PXI - EXI| > TH_GAP (15),
    reliably firing TC-03 even when called immediately after TC-01/02 engagement."""
    s = st.session_state
    s.tick += 1
    # PXI: small recovery boost (patient-side resilience effect)
    s.PXI = float(np.clip(s.PXI + 5.0, 35, 100))
